...
- **응답이 느리다** (평소 90ms지만 간헐적으로 4초). 타임아웃을 30초로 잡았다.
- **arrtime은 약 2분마다만 갱신된다.** 받은 값은 이미 0~120초 낡아 있고 갱신 시
  크게 점프한다(1823→1453초). 그래서 도착정보는 캐시하지 않고 항상 직접 조회한다 —
  캐시하면 실시간 값과 세대가 달라져 수분씩 벌어진다.
